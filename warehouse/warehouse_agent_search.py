    ...
    def __init__(self, environment: S):
        ...
        for a in self.forklifts:
            for p in self.products:
                self.pairs.append(Pair(a, p, 0))#foi acrescentado o 0 para o custo

        for i in range(len(self.products) - 1):
            for j in range(i + 1, len(self.products)):
                self.pairs.append(Pair(self.products[i], self.products[j], 0))#foi acrescentado o 0 para o custo

        for p in self.products:
            self.pairs.append(Pair(p, self.exit, 0))#foi acrescentado o 0 para o custo

        for a in self.forklifts:
            self.pairs.append(Pair(a, self.exit,0))
